class Chantier < ApplicationRecord
  #belongs_to :ouvrage
    ...
    def self.import_association(file)
        CSV.foreach(file.path) do |row|
            @ouvrage = Ouvrage.find_by_name(row[1])
            if !@ouvrage.nil? && Chantier.all.where('code = ?', row[0]).count > 0 
                Chantier.all.where('code = ?', row[0]).each do |chantier|
                    chantier.ouvrage_id = @ouvrage.id
                    chantier.save
                end
            end
        end 
